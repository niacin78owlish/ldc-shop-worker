# LDC Store - Linux DO 积分商城 (Workers v2)
...
- **库存锁定与超时释放**：进入支付页锁定 1 分钟，5 分钟未支付自动取消并释放库存。
- **限购**：按已支付次数限制购买。
- **商城体验**：搜索/分类筛选、公告栏、Markdown 商品描述、评分与评论、访客数量统计。
- **社交分享**：支持 X/Twitter、Facebook、Telegram、WhatsApp、Line 和复制链接。
- **Linux DO 深度集成**：
    - 支持 **Linux DO Connect** (OIDC) 一键登录。
    - 支持 **Linux DO Credit** (EasyPay 协议) 积分支付。
- **管理后台**：商品上下架/排序/限购、库存管理、订单查询、退款、公告配置。
- **安全可靠**：无服务器维护烦恼，数据通过 Cloudflare D1 保障。

> 定时清理由 `wrangler.toml` 的 `crons` 配置触发（默认每分钟一次），可按需调整。
...
4. 在后台你可以：
    - **Products**: 创建商品、设置上下架、排序、限购、图片外链。

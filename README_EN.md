...
## ☁️ Cloudflare Workers One-Click Deploy

[![Deploy to Cloudflare Workers](https://deploy.workers.cloudflare.com/button)](https://deploy.workers.cloudflare.com/?repo=https://github.com/chatgptuk/ldc-shop)

> If you are using a fork, replace the repo URL in the button with your own.

After deployment, finish setup:
1. **Create a D1 database** and put the `database_id` into the root `wrangler.toml`.
2. **Initialize schema**:
   ```bash
   wrangler d1 execute ldc-shop-db --file=_workers_v2/schema.sql
   ```
3. **Set Secrets** (Cloudflare Dashboard or `wrangler secret put`):
   - `MERCHANT_ID`, `MERCHANT_KEY`
   - `OAUTH_CLIENT_ID`, `OAUTH_CLIENT_SECRET`, `OAUTH_REDIRECT_URI`
   - `ADMIN_USERS`
4. (Optional) Bind a custom domain.

> The root `wrangler.toml` points to `_workers_v2/worker.js` for one-click deploy.

...
> 如果你使用的是 fork 仓库，请把按钮中的 repo 链接替换成你自己的地址。

部署后请按以下步骤完成配置：
1. **创建 D1 数据库**，并把生成的 `database_id` 填入根目录 `wrangler.toml`。
2. **初始化表结构**：
   ```bash
   wrangler d1 execute ldc-shop-db --file=_workers_v2/schema.sql
   ```
3. **设置 Secrets**（Cloudflare Dashboard 或 `wrangler secret put`）：
   - `MERCHANT_ID`, `MERCHANT_KEY`
   - `OAUTH_CLIENT_ID`, `OAUTH_CLIENT_SECRET`, `OAUTH_REDIRECT_URI`
   - `ADMIN_USERS`
4. （可选）绑定自定义域名。

> 根目录 `wrangler.toml` 已指向 `_workers_v2/worker.js`，用于一键部署。
